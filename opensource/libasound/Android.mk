...
# PRODUCT_COPY_FILES2 += $(foreach p,$(copy_from),$(LOCAL_ALSA_CONF_DIR)/$(p):$(TARGET_ALSA_CONF_DIR)/$(p))
...
LOCAL_POST_PROCESS_COMMAND2 := \
	@mkdir -p $(TARGET_ALSA_CONF_DIR); \
	for f in $(copy_from); do \
		ldir=$$(dirname $(TARGET_ALSA_CONF_DIR)/$${f}); \
		mkdir -p $${ldir}; \
		cp -a $(LOCAL_ALSA_CONF_DIR)/$${f} $${ldir}; \
	done
