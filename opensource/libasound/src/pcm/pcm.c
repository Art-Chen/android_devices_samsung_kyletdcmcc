				     ...
				     snd_pcm_xfer_areas_func_t func)
{
	...
	_again:
		...
		if (avail == 0) {
			...
			err = snd_pcm_wait(pcm, 1000); // we change it for 1s timeout, for recording [luther.ge]
			if (err <= 0) {
                if (err == 0) err = -EAGAIN;
				break;
            }
				      ...
				      snd_pcm_xfer_areas_func_t func)
{
	...
	_again:
		...
		if ((state == SND_PCM_STATE_RUNNING &&
			...
			err = snd_pcm_wait(pcm, 1000); // we change it for 1s timeout, for playing [luther.ge]
			if (err < 0) {
                if (err == 0) err = -EAGAIN;
				break;
            }
